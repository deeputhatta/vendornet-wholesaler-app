...
export default function DashboardScreen({ navigation }) {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const lowStock = allListings.filter(l => l.stock_qty < 10).length;
  ...
  return (
    ...
    >
      ...
      {/* ══════════════════════════════
          SECTION 2 — GREETING (medium green)
      ══════════════════════════════ */}
      ...
      <View style={[styles.contentCard, { backgroundColor: c.background }]}>
        <Animated.View style={{
          ...
          <View style={styles.statsGrid}>
            {[
              { icon: '📦', label: 'Pending Orders', value: stats?.pending ?? 0, color: '#F2C94C', bg: '#2A1F00', onPress: () => navigation.navigate('Orders') },
              { icon: '🏷', label: 'Active Listings', value: stats?.totalListings ?? 0, color: '#30D158', bg: '#003A10' },
              { icon: '⚠️', label: 'Low Stock', value: stats?.lowStock ?? 0, color: '#FF9500', bg: '#2A1500' },
              { icon: '💰', label: 'Commission', value: '1%', color: '#BF5AF2', bg: '#1A0A2E' },
            ].map((s, i) => (
              <TouchableOpacity
                key={i}
          ...
          <View style={styles.actionsGrid}>
            {[
              { icon: '📦', label: 'Manage\nOrders', color: '#0A84FF', bg: '#001830', border: '#0A2A50', onPress: () => navigation.navigate('Orders'), badge: stats?.pending > 0 ? stats.pending : null },
              { icon: '📈', label: 'Analytics', color: '#30D158', bg: '#003A10', border: '#0A3A20', onPress: () => navigation.navigate('Analytics') },
              { icon: '👤', label: 'My\nProfile', color: '#BF5AF2', bg: '#1A0A2E', border: '#2A1A4E', onPress: () => navigation.navigate('Profile') },
              { icon: '🧾', label: 'Upload\nInvoice', color: '#FF9500', bg: '#2A1500', border: '#3A2500', onPress: () => navigation.navigate('Orders') },
            ].map((a, i) => (
              <TouchableOpacity
                key={i}
              ...
              <View style={styles.orderContent}>
                ...
                <View style={styles.orderActions}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('Orders')}
                  >
                    <Text style={[styles.detailText, { color: c.primary }]}>View Details →</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </TouchableOpacity>
